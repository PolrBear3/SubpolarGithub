...
public class FoodState_Controller : MonoBehaviour
{
    ...
    private void Awake()
    {
        for (int i = 0; i < _stateIndicator.childCount; i++)
        {
            Transform stateBox = _stateIndicator.GetChild(i);
            Destroy(stateBox.gameObject);
        }
    }
    ...
    // State Boxes Position Update
    private void StateBoxes_PositionUpdate()
    {
        _boxPositionCount = 0;
    ...
    public void CurrentState_Transparency(bool isTransparent)
    {
        for (int i = 0; i < currentStateBoxes.Count; i++)
        {
    ...
    public void CurrentState_Transparency(FoodState_Type stateType, bool isTransparent)
    {
    ...
    // Clear All State Boxes
    public void Clear_State()
    {
        for (int i = 0; i < currentStateBoxes.Count; i++)
        {
            currentStateBoxes[i].Clear_State();
        }
    }
    // Clear Specific State Boxes
    public void Clear_State(FoodState_Type stateType)
    {
        if (Has_State(stateType) == true)
        {
